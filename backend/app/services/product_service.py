...
class ProductService:
    ...
    def get_all_products(self) -> ProductListResponse:
        ...
        products_response = [ProductResponse.model_validate(product)
    ...
    def get_products_by_category(self, category_id: int) -> ProductListResponse:
        ...
        if not category:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Category with category_id {category_id} not found"
            )
    ...
    def create_product(self, product_data: ProductCreate) -> ProductResponse:
        ...
        if not category:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Category with category_id {product_data.category_id} not found"
            )
